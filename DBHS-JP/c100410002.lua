...
function c100410002.initial_effect(c)
	...
	e1:SetCategory(CCATEGORY_TOHAND+CATEGORY_SEARCH)
	...
	e1:SetOperation(c100410002.spr)
	c:RegisterEffect(e1)
...
function c100410002.filter(c,e,tp)
	return c:IsSetCard(0x219) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c100410002.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(c100410002.filter,tp,LOCATION_DECK,0,1,nil,e,tp)
	...
	local g=Duel.SelectTarget(tp,c100410002.filter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c100410002.spcon2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
