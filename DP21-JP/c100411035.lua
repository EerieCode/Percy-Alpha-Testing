...
function s.atop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsAttackCostPaid()~=2 and e:GetHandler():IsLocation(LOCATION_MZONE) then
        ...
        local sg=aux.SelectUnselectGroup(g,e,tp,2,2,nil,1,tp,HINTMSG_TOGRAVE,Duel.IsAttackCostPaid()==0,Duel.IsAttackCostPaid()==0)
        if #sg==2 then
...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,s.thfilter,tp,LOCATION_GRAVE,0,1,1,nil) --might need necrovalley stuff?
