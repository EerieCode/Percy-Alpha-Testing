...
function card.becomeNormal(e, tp, eg, ep, ev, re, r, rp)
    ...
    Duel.RegisterEffect(e2, tp)
    card.applyNewChallengeReset(e2)
end
...
function card.noSummonLowATK()
    ...
    card.applyNewChallengeReset(e3)
end
...
function card.rescon(mzc, szc, g)
    return function(sg, e, tp, mg)
        ...
        elseif szc == 1 then
            ...
            if g:IsExists(Card.IsType, 1, nil, TYPE_SPELL | TYPE_TRAP) then
                return bg:IsExists(card.raux1, 1, nil, bg, g, false) and
...
function card.raux1(c, bg, g, mfield)
    local bool =
        g:IsExists(card.raux2, 1, c, (TYPE_SPELL | TYPE_TRAP) & (~c:GetType()), not c:IsType(TYPE_FIELD), mfield)
    return (bool and
        bg:IsExists(card.raux2, 1, c, (TYPE_SPELL | TYPE_TRAP) & (~c:GetType()), not c:IsType(TYPE_FIELD), mfield)) or
