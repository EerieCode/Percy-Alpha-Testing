...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(aux.TRUE,tp,LOCATION_MZONE,LOCATION_MZONE,1,c) 
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,CARD_IGNISTER_TOKEN,0x135,0x4011,0,0,1,RACE_CYBERSE,ATTRIBUTE_DARK) end
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)>0
		...
		and Duel.IsPlayerCanSpecialSummonMonster(tp,CARD_IGNISTER_TOKEN,0x135,0x4011,0,0,1,RACE_CYBERSE,ATTRIBUTE_DARK) then
		local token=Duel.CreateToken(tp,CARD_IGNISTER_TOKEN)
