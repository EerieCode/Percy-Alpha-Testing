...
function s.opttarget(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and s.optfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(s.optfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,s.optfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
