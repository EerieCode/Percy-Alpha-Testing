...
function c100409024.tfop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.MoveToField(tc,tp,tp,LOCATION_SZONE,POS_FACEUP,true) and (c:IsSummonableCard() or c:IsAbleToGrave()) 
		and Duel.SelectYesNo(tp,aux.Stringid(100409024,1)) then
		Duel.BreakEffect()
		local e1=Effect.CreateEffect(c)
		...
		if c:IsSummonable(true,nil) and c:IsAbleToGrave() then
			op=Duel.SelectOption(tp,aux.Stringid(100409024,2),aux.Stringid(100409024,3))
		elseif c:IsAbleToGrave() then
			op=Duel.SelectOption(tp,aux.Stringid(100409024,2))
		else
			op=Duel.SelectOption(tp,aux.Stringid(100409024,3))+1
		end
		e:SetLabel(op)
		if op==0 then
