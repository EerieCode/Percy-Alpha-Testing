...
function s.initial_effect(c)
	...
	e0:SetTarget(Auxiliary.LinkTarget(aux.FilterBoolFunctionEx(Card.IsType,TYPE_NORMAL),1,1,nil))
	e0:SetOperation(Auxiliary.LinkOperation(aux.FilterBoolFunctionEx(Card.IsType,TYPE_NORMAL),1,1,nil))
...
function s.linkcon(e,c)
	...
	local mg=g:Filter(Auxiliary.LConditionFilter,nil,aux.FilterBoolFunctionEx(Card.IsType,TYPE_NORMAL),c,tp)
	local mustg=Auxiliary.GetMustBeMaterialGroup(tp,g,tp,c,mg,REASON_LINK)
	if mustg:IsExists(aux.NOT(Auxiliary.LConditionFilter),1,nil,aux.FilterBoolFunctionEx(Card.IsType,TYPE_NORMAL),c,tp) then return false end
	local emt,tg=aux.GetExtraMaterials(tp,mustg+mg,c,SUMMON_TYPE_LINK)
	local sg=mustg
	return (mg+tg):Includes(mustg) and ((#sg>=1 and aux.LCheckGoal(tp,mustg,c,1,aux.FilterBoolFunctionEx(Card.IsType,TYPE_NORMAL),nil,{})) or (mg+tg):IsExists(Auxiliary.LCheckRecursive,1,sg,tp,sg,(mg+tg),c,1,1,aux.FilterBoolFunctionEx(Card.IsType,TYPE_NORMAL),nil,mg,emt))
end
