...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.SelectReleaseGroupCost(tp,aux.TRUE,1,3,false,nil,e:GetHandler())
...
function s.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		local sel=0
		if eff==1 and Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)) then sel=0 end
		if ct>0 and eff==2 and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil) then sel=sel+1 end
		if eff==3 and Duel.IsExistingMatchingCard(s.svfilter,tp,LOCATION_GRAVE,0,1,nil) then sel=sel+2 end
		e:SetLabel(sel)
		return sel~=0 and true
	end
