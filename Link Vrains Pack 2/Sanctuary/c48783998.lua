...
function c48783998.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		if not (Duel.IsExistingMatchingCard(c48783998.envfilter,0,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) or Duel.IsEnvironment(CARD_SANCTUARY_SKY)) then
...
function c48783998.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if not (Duel.IsExistingMatchingCard(c48783998.envfilter,e:GetHandlerPlayer(),LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) or Duel.IsEnvironment(CARD_SANCTUARY_SKY)) then
