...
function s.eqfilter(c)
	return c:IsSetCard(0x7e) and c:IsType(TYPE_MONSTER) and not c:IsForbidden()
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local b1=Duel.IsExistingTarget(s.filter,tp,LOCATION_MZONE,0,1,nil) and Duel.IsExistingMatchingCard(s.eqfilter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,nil) and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(s.eqfilter,tp,LOCATION_DECK+LOCATION_EXTRA,0,nil)
