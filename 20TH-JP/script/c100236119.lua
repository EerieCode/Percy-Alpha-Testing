...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g1>0 and #g2>0 then
		Duel.BreakEffect()
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g3=Duel.SelectMatchingCard(tp,Card.IsAbleToHand,tp,0,LOCATION_DECK,4,4,nil)
		Duel.Hint(HINT_SELECTMSG,1-tp,HINTMSG_ATOHAND)
		local g4=Duel.SelectMatchingCard(1-tp,Card.IsAbleToHand,1-tp,0,LOCATION_DECK,4,4,nil)
		if #g3==4 and #g4==4 then
			local ga=g1+g3
			local gb=g2+g4
			local ca=ga:RandomSelect(1)
			Duel.ConfirmCards(1-tp,ca)
			Duel.SendtoHand(ca,tp,REASON_EFFECT)
			local dg=ga-ca
			local cb=gb:RandomSelect(1)
			Duel.ConfirmCards(tp,cb)
			Duel.SendtoHand(ba,1-tp,REASON_EFFECT)
			dg=dg+(gb-cb)
