...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local bx=g:IsExists(Card.IsType,1,nil,TYPE_XYZ) and rg:IsExists(Card.IsType,1,nil,TYPE_SPELL+TYPE_TRAP)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:IsExists(Card.IsType,1,nil,TYPE_SYNCHRO) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		og:Merge(rg:Filter(s.exfilter,nil):RandomSelect(tp,3))
	end
	if g:IsExists(Card.IsType,1,nil,TYPE_XYZ) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		og:Merge(rg:FilterSelect(tp,Card.IsType,1,1,og,TYPE_SPELL+TYPE_TRAP))
	end
