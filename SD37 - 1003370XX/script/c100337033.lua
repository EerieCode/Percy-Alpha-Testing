...
function s.posop1(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.SelectMatchingCard(Card.IsFacedown,tp,LOCATION_MZONE,0,1,1,nil)
	if #g>0 then
...
function s.posop2(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.SelectMatchingCard(aux.FilterFaceupFunction(Card.IsCanTurnSet),tp,LOCATION_MZONE,0,1,1,nil)
	if #g>0 then
		Duel.HintSelection(g)
