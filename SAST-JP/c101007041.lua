...
function c101007041.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT) then
		...
		if tc:IsRace(RACE_ZOMBIE) and b1:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(101007041,1)) then
		...
		if tc:IsAttribute(ATTRIBUTE_FIRE) and b2:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(101007041,2)) then
			local t2=b2:Select(tp,1,1,nil)
            Duel.HintSelection(t2)
            Duel.Destroy(t2,REASON_EFFECT)
		end
		if tc:IsType(TYPE_SYNCHRO) and b3:GetCount()>0  and Duel.SelectYesNo(tp,aux.Stringid(101007041,3)) then
