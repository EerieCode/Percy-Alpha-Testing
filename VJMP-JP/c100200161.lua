...
function s.initial_effect(c)
	c:EnableCounterPermit(0x1150)
	c:SetCounterLimit(0x1150,2)
	--place counter
...
function s.countercon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()==tp and e:GetHandler():GetCounter(0x1150)<2
end
function s.counterop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsFaceup() and c:IsRelateToEffect(e) then
		c:AddCounter(0x1150,1)
	end
end
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	local at=Duel.GetAttacker()
	return at:IsControler(1-tp) and e:GetHandler():GetCounter(0x1150)>=1
end
...
function s.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return e:GetHandler():GetCounter(0x1150)>=2 and #g>0 end
